...
public class ActivateObject : MonoBehaviour
{
    ...
    {
        if (col.CompareTag("Player") && col.gameObject.name == "Woodle Character" && switchToCam && cartToMove)
        {
    ...
    {
        if (col.CompareTag("Player") && col.gameObject.name == "Woodle Character" && switchToCam != null && cartToMove)
        {
            dCartComp.m_Speed = 0f;
            switchToCam.VirtualCameraGameObject.SetActive(false);
        }
