...
namespace ICode.Actions.Photon{
	...
	public class InstantiateSceneObject : StateAction {
		...
		private void DoInstantiate(){
			store.Value = PhotonNetwork.InstantiateSceneObject (prefabName.Value, FsmUtility.GetPosition(target, position), target.Value != null && euler.IsNone?target.Value.transform.rotation:Quaternion.Euler(euler.Value),(byte) group.Value,null);
		}
	}
}
#endif