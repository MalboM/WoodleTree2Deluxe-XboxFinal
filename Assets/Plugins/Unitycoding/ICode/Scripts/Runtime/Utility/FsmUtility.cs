...
namespace ICode
{
	public static class FsmUtility
	{
		...
		private static object CopyFields (object source)
		{
			...
			Type type = source.GetType ();
			if (type.IsValueType () || type == typeof(string) || type == typeof(AnimationCurve) || type == typeof(GameObject) || typeof(UnityEngine.Object).IsAssignableFrom (type) && !typeof(FsmVariable).IsAssignableFrom (type)) {
				//	Debug.Log(source);
				return source;
			} else if (type.IsSubclassOf (typeof(ScriptableObject))) {
