...
public class SmoothLookAtWoodle : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (woodle)
        {
            if (smooth)
            {
                ...
                Quaternion rotation = Quaternion.LookRotation(woodle.position - _myTransform.position);
                _myTransform.rotation = Quaternion.Slerp(_myTransform.rotation, rotation, Time.deltaTime * damping);
            }
            else { //Just look at
                _myTransform.rotation = Quaternion.FromToRotation(-Vector3.forward, (new Vector3(woodle.position.x, woodle.position.y, woodle.position.z) - _myTransform.position).normalized);

                float distance = Vector3.Distance(woodle.position, _myTransform.position);
