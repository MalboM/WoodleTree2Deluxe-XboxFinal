Shader "WoodleTree/Unlit/Triplanar/Normal/triplanar_projection_diffuse_normal"
{
	Properties
	{
		...
		_TileF ("Tiling", Range(0.05, 20)) = 0.5
		...
		_RimSharpnessF("Rimlight Sharpness", Range(0.0, 2.0)) = 0.5
		_RimIntensityF("Rimlight Intensity", Range(0.0, 0.2)) = 0.25
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				half4 mainCol = tex2D(_MainTex, i.uv.yz) * i.normalWorld.w +
				...
				fixed4 col = (mainCol * _TintColor * 2.0) + ((_RimColor * i.uv.w) * _RimIntensityF);
